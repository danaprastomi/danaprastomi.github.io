...
@section('subtitle', 'Buat gambar carousel banner baru untuk halaman beranda')
...
@section('content')
    <div class="max-w-4xl bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-slate-200/80 flex items-center justify-between">
            <h3 class="font-bold text-slate-800 text-lg">Form Tambah Banner</h3>
        ...
        <form method="POST" action="{{ route('admin.sliders.store') }}" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" required value="{{ old('title') }}" placeholder="Contoh: CV. Cakrawala Langit Semesta"
                ...
                <div>
                    ...
                    <input type="text" id="subtitle" name="subtitle" required value="{{ old('subtitle') }}" placeholder="Contoh: General Supplier Terpercaya"
                        class="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:border-sky-500 focus:ring-1 focus:ring-sky-500 focus:outline-none transition-all">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                    ...
                    <div>
                        <label for="image" class="image-preview-area flex flex-col items-center justify-center p-8 rounded-2xl cursor-pointer text-center">
                            ...
                            <span class="text-sm font-bold text-slate-600">Klik untuk upload gambar</span>
                            <span class="text-xs text-slate-400 mt-1">Maks. 2MB (format: PNG, JPG, JPEG, WEBP)</span>
                            <input type="file" id="image" name="image" required class="hidden" accept="image/*" onchange="previewImage(event)">
                        </label>
