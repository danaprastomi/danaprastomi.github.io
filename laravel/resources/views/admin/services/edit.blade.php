...
@section('subtitle', 'Ubah data katalog barang supplier atau jasa yang sudah ada')
...
@section('content')
    <div class="max-w-4xl bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-slate-200/80 flex items-center justify-between">
            <h3 class="font-bold text-slate-800 text-lg">Form Edit Produk</h3>
        ...
        <form method="POST" action="{{ route('admin.services.update', $service->id) }}" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" required value="{{ old('name', $service->name) }}" placeholder="Contoh: Lampu Philips 18 Watt"
                ...
                <div>
                    ...
                    <input type="text" id="category" name="category" required value="{{ old('category', $service->category) }}" placeholder="Ketik atau pilih kategori..." list="category-list"
                        class="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:border-sky-500 focus:ring-1 focus:ring-sky-500 focus:outline-none transition-all">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" placeholder="Detail spesifikasi barang, ukuran, merk, dll..."
                    class="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:border-sky-500 focus:ring-1 focus:ring-sky-500 focus:outline-none transition-all resize-y">{{ old('description', $service->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 items-center">
                ...
                <div>
                    ...
                    <input type="text" id="icon" name="icon" value="{{ old('icon', $service->icon) }}" placeholder="Contoh: lightbulb"
                        class="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:border-sky-500 focus:ring-1 focus:ring-sky-500 focus:outline-none transition-all">
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Foto / Gambar Produk</label>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                    ...
                    <div>
                        <label for="image" class="image-preview-area flex flex-col items-center justify-center p-8 rounded-2xl cursor-pointer text-center">
                            ...
                            <span class="text-sm font-bold text-slate-600">Klik untuk ganti foto</span>
                            <span class="text-xs text-slate-400 mt-1">Biarkan kosong jika tidak ingin mengubah foto</span>
                    ...
                    <div>
                        ...
                        <div class="relative w-full aspect-video rounded-2xl overflow-hidden bg-slate-50 border border-slate-200/60 shadow-sm flex items-center justify-center">
                            @if($service->image)
                                <img id="image-preview" src="{{ Storage::url($service->image) }}" alt="Preview" class="w-full h-full object-cover">
