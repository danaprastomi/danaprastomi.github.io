...
@section('title', 'Tambah Variabel Profil')
@section('subtitle', 'Buat variabel informasi baru untuk profil perusahaan')
...
@section('content')
    <div class="max-w-4xl bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-slate-200/80 flex items-center justify-between">
            <h3 class="font-bold text-slate-800 text-lg">Form Tambah Variabel</h3>
        ...
        <form method="POST" action="{{ route('admin.company-profile.store') }}" class="p-6 sm:p-8 space-y-6">
            @csrf
            
            {{-- Key --}}
            <div>
                <label for="key" class="block text-sm font-semibold text-slate-700 mb-2">Kunci Variabel (Key) *</label>
                <input type="text" id="key" name="key" required value="{{ old('key') }}" placeholder="Contoh: alamat, email, visi, whatsapp"
                ...
                <p class="text-slate-400 text-xs mt-1.5 leading-normal">Gunakan huruf kecil tanpa spasi (disarankan menggunakan snake_case, contoh: `whatsapp_nomor` atau `alamat_cabang`).</p>
            ...
            <div>
                ...
                <textarea id="value" name="value" rows="6" required placeholder="Tulis isi informasi atau nilai dari variabel ini..."
                    class="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:border-sky-500 focus:ring-1 focus:ring-sky-500 focus:outline-none transition-all resize-y">{{ old('value') }}</textarea>
